...
resample1_ests <- lm(diamonds[resample1,]$price~diamonds[resample1,]$carat)$coefficients
resample1_ests
```
...
# for (b in 1:B) {
#   resampled_ests[b, ] <- coefficients(
#     lm(diamonds[resampled_values[b,],]$price~
#          diamonds[resampled_values[b, ], ]$carat))
# }
# head(resampled_ests)

# Am I clever enough? 
resampled_ests <- t(apply(resampled_values, 1, function(x) coefficients(lm(diamonds[x,]$price ~ diamonds[x,]$carat))))
colnames(resampled_ests) <- c("Intercept_Est", "Slope_Est")
...
hist(resampled_ests[, 2], xlab = "beta1", main = "histogram of bootstrap of beta1")
```
...
Cl <- quantile(diff_estimates, .025) + lm0$coefficients[2]
Cu <- quantile(diff_estimates, .975) + lm0$coefficients[2]
int <- c(Cl, Cu)
int
```
...
```{r}
re.025 <- quantile(resampled_ests[,2], .025)
re.975 <- quantile(resampled_ests[,2], .975)
c(re.025, re.975)
```
